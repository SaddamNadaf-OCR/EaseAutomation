...
public class StandardRequestCreationUseCaseVMs8 extends SeleniumTestBase {
			...
			if (!Utility.isExecutable(getClass().getSimpleName(), excel)) {
				...
				if (RunMode.equalsIgnoreCase("y")) {
					...
					for (int i = 0; i < title.size(); i++) {
						visitors.add_Individual(title.get(i), fname.get(i), MiddleName, lname.get(i), /*company.get(i),*/
	...
	public Object[][] getData() {
		return Utility.getData("UseCase8", excel);
	}
}
