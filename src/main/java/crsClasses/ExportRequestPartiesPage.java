...
public class ExportRequestPartiesPage extends GenericMethods {
	...
	public static final String OCR_ExportRequest_PartiesVendorConsigneeID="(//input[@name='userDTO.userCode'])[1]";
	public static final String OCR_PartiesGrid = "//div[@id='gbox_gridconsignee']";

	/************************* constructor ***********************/
	public ExportRequestPartiesPage(WebDriver driver, ExtentTest test) {
		this.driver = driver;
		this.test = test;
	}
	 ...
	 * clickElement(driver, test, OCR_ExportRequest_PartiesSearchByCodePartyLink,
	 * "Click on  Search by Code"); selectByVisibleText(driver, test,
	 * OCR_ExportREquest_PartiesCustomerStatus, "Select Approve Status",
	 * CustomerStatus); clickElement(driver, test,
	 * OCR_ExportRequest_PartiesSearchBtn, "Search Approved Customer"); String
	 * CustomerP = "//td[contains(text(),'" + customer + "')]";
	 * System.out.println(CustomerP); clickElement(driver, test, CustomerP,
	 * "Select a Customer"); clickElement(driver, test,
	 * OCR_ExportRequest_PartiesSelectBtn, "click on Select Button"); }
	 */
	
	public void addPartyCustomer(String CustomerStatus, String customer) {
		clickElement(driver, test, OCR_ExportRequest_PartiesSearchByCodePartyLink, "Click on  Search by Code");
		try {
			typeText(driver, test, OCR_ExportRequest_PartiesVendorConsigneeID, "Entering Vendor/Consignee ID :", customer);
			clickElement(driver, test, OCR_ExportRequest_PartiesSearchBtn, "Clicking on Search Button :");
			mouseOverAndClick(driver, test, "//td[contains(text(),'"+customer+"')]", "Selecting Vendor/Consignee ID :");
			clickElement(driver, test, OCR_ExportRequest_PartiesSelectBtn, "Clicking on Select Button :");
		}
		catch(Exception e) {
			testFailSshot(driver, test);
			e.printStackTrace();
			clickElement(driver, test, "(//button[@class='btn-close'])[1]", "Clicking on Cancel Button :");
		}
	}
