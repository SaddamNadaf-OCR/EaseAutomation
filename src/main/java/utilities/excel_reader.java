...
public class excel_reader {
	...
	// returns the rows count in a sheet
	...
	// Setting VID in the excel
	public void set_VID(String vid, int row) throws Exception {
		FileInputStream fs = new FileInputStream(new File(path));
		XSSFWorkbook workbook = new XSSFWorkbook(fs);
		XSSFSheet st = workbook.getSheet("RapidRegistration");
		XSSFRow r1 = st.getRow(0);
		XSSFCell cell1;
		int i = r1.getLastCellNum();
		String str;
		for (int k = 0; k < i; k++) {
			cell1 = r1.getCell(k);
	...
	// Copy VID and Moving VID to second excel sheet
	public void copy_SetVID() throws Exception {
		FileInputStream fs = new FileInputStream(new File(path));
		XSSFWorkbook workbook = new XSSFWorkbook(fs);
		XSSFSheet st = workbook.getSheet("VisitorFrontDesk");
		...
		XSSFRow r1 = st.getRow(0);
		XSSFCell cell1;
		int l = r1.getLastCellNum();
		String st1;
		for (int k = 0; k < l; k++) {
			cell1 = r1.getCell(k);
			st1 = cell1.getStringCellValue();
			if (st1.equalsIgnoreCase("VID")) {
				// System.out.println(k);
				l = k;
				break;
			}
		...
		XSSFSheet st11 = workbook.getSheetAt(2);
		XSSFCell cell11 = null;
		XSSFRow r11 = null;
		for (int i = 0; i < rownum; i++) {
			r11 = st11.getRow(i + 1);
			cell11 = r11.createCell(0);
			cell11.setCellValue(str[i]);
		}
		FileOutputStream fileOut = new FileOutputStream(path);
		workbook.write(fileOut);
		fileOut.close();
