 ...
 * Descriptions : Report Shipment Summary Page
 * changed by : Nil
 * changed date : Nil
...
public class ReportShipmentSummaryPage extends GenericMethods {
	public WebDriver driver;
	public ExtentTest test;

	/* ******************************** LOCATORS ******************************** */
	public static final String OCR_SEARCH = "(//button[contains(text(),'Search')])";
	public static final String OCR_ExportToExcel = "//input[@value='Export To Excel']";
	public static final String OCR_ExportToPDF = "//input[@value='Export to PDF']";
	...
	public static final String OCR_ShipmentStatus = "//select[@name='shipment.shipmentWorkStatus']";
	...
	public static final String OCR_USPPIIDandType = "//input[@name='shipment.usppiNumber']";
	public static final String OCR_USPPIIDandType1 = "//select[@name='shipment.filerIdType']";
	public static final String OCR_fromCreatedDate = "//input[@name='fromCreatedDate']";
	public static final String OCR_toCreatedDate = "//input[@name='toCreatedDate']";
	public static final String OCR_createdBy = "//select[@name='shipment.createdBy']";
	public static final String OCR_InvoiceNo = "//input[@name='invoiceNo']";
	...
	public ReportShipmentSummaryPage(WebDriver driver, ExtentTest test) {
		this.driver = driver;
		this.test = test;
	}
	/* ******************************* Action ****************************** */
	...
	// Search by parameter
	public void parameter(String Sbu, String ShipmentNo, String Workstatus, String AESStatus, String AESOption,
			String InvoiceNo) {
		...
		selectByVisibleText(driver, test, OCR_ShipmentStatus, "Select Shipment workstatus", Workstatus);
		selectByVisibleText(driver, test, OCR_AESStatus, "Select AES Status", AESStatus);
	...
	// Search by parameter
		public void parameter(String Country, String Sbu, String ShipmentNo, String ExporterCode, String EEINo, 
				String WorkStatus, String AESStatus, String Ult_ConsigneeName, String Inter_ConsigneeName, String UltDestination, 
				String ForwardingAgentName, String carrierName, String DateOfExportFrom, String DateOfExportTo, String SLIDateFrom, 
				String SLIDateTo, String billOfLading, String bookingNo, String hazardous, String routingInfo, String reportMonth, 
				String carnetNo, String mawbNo, String hawbNo, String aesOption, String awbNo, String prepaid, String collect, 
				String USPPIIDandType, String USPPIIDandType1, String fromCreatedDate, String toCreatedDate, String createdBy, String Invoice_No, 
				...
				String htsNo, String productClassification, String itarNo, String sortBy, String direction) {
			...
			if(WorkStatus.length() > 0) {
				selectByVisibleText(driver, test, OCR_ShipmentStatus, "Select Work Status", WorkStatus);
			}
			...
			if(routingInfo.length() > 0) {
				typeText(driver, test, OCR_AESExemptionStatement, "Enter AES Exemption Statement :", routingInfo);
			}
			...
			if(USPPIIDandType.length() > 0) {
				selectByVisibleText(driver, test, OCR_USPPIIDandType, "Select USPPI ID and Type :", USPPIIDandType);
			}
			if(USPPIIDandType1.length() > 0) {
				selectByVisibleText(driver, test, OCR_USPPIIDandType1, "Selecting USPPIIDandType1 :", USPPIIDandType1);
			}
			if(fromCreatedDate.length() > 0) {
				clickElement(driver, test, OCR_fromCreatedDate, "Clicking on from Created Date :");
				try {
					if(fromCreatedDate.equalsIgnoreCase("Today")) {
						clickElement(driver, test, OCR_Today, "Clicking on Today :");
					}else {
						dateOfMonthName(driver, test, "Selecting from Created Date :", fromCreatedDate);
					}
				}catch(Exception e) {
					testFailSshot(driver, test);
					test.log(LogStatus.FAIL, "Enable to Select the from Created Date :");
				}
			}
			if(toCreatedDate.length() > 0) {
				clickElement(driver, test, OCR_toCreatedDate, "Clicking on to Created Date :");
				try {
					if(toCreatedDate.equalsIgnoreCase("Today")) {
						clickElement(driver, test, OCR_Today, "Clicking on Today :");
					}else {
						dateOfMonthName(driver, test, "Selecting to Created Date :", toCreatedDate);
					}
				}catch(Exception e) {
					testFailSshot(driver, test);
					test.log(LogStatus.FAIL, "Enable to Select the to Created Date :");
				}
			}
			...
			if(Invoice_No.length() > 0) {
				typeText(driver, test, OCR_InvoiceNo, "Enter Invoice No", Invoice_No);
			}
			if(glaCode.length() > 0) {
